                ...
                <div class="d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
                    ...
                    <div class="overflow-auto" style="white-space:nowrap; -webkit-overflow-scrolling:touch; flex:1;">
                        ...
                        <a href="{{ route('owner.products.index', ['search' => $search ?? '']) }}"
                            ...
                            All
                            <span class="ms-2"
                                style="background:#F5F0E5;color:#A4823B;border-radius:10px;padding:4px 8px;font-size:0.85rem;">{{ $totalProductsCount }}</span>
                        </a>
                        ...
                        @foreach ($categories as $cat)
                            ...
                            <a href="{{ route('owner.products.index', ['category' => $catId, 'search' => $search ?? '']) }}"
                                ...
                                {{ $cat->name }}
                                <span class="ms-2"
                                    style="background:#F5F0E5;color:#A4823B;border-radius:10px;padding:4px 8px;font-size:0.85rem;">
                                    {{ $productCount }}
                                </span>
                            </a>
                        @endforeach
                ...
                <table class="table mb-0 align-middle">
                    <thead style="background:#F5F0E5;">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-center" style="color:#4b3028; font-weight:700; width:110px;">Category
                            </th>
                            <th class="px-3 py-3 text-end" style="color:#4b3028; font-weight:700; width:100px;">Price</th>
                            <th class="px-3 py-3 text-center" style="color:#4b3028; font-weight:700; width:100px;">Recipe
                            </th>
                            <th class="px-3 py-3 text-center" style="color:#4b3028; font-weight:700; width:100px;">Status
                            </th>
                            <th class="px-3 py-3 text-center" style="color:#4b3028; font-weight:700; width:190px;">Actions
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr style="border-bottom:1px solid #E9E6E2;">
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    @else
                                        ...
                                        <div
                                            style="width:45px;height:45px;background:#F5F0E5;border-radius:8px;display:flex;align-items:center;justify-content:center;">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                viewBox="0 0 24 24" fill="none" stroke="#A4823B" stroke-width="2"
                                ...
                                <td class="px-3 py-2 text-center">
                                    ...
                                    @endphp
                                    <span class="badge"
                                        style="background:{{ $colors['bg'] }};color:{{ $colors['text'] }};font-weight:600;padding:5px 10px;border-radius:6px;font-size:0.8rem;">
                                ...
                                <td class="px-3 py-2 text-center">
                                    ...
                                    @if ($ingredientCount > 0)
                                        <span class="badge"
                                            style="background:#E6F9EE;color:#2D7A3B;font-weight:600;padding:5px 10px;border-radius:6px;font-size:0.8rem;">
                                            <i class="bi bi-box-seam me-1"></i>{{ $ingredientCount }}
                                        </span>
                                    @else
                                        <span class="badge"
                                            style="background:#F5F5F5;color:#9E9E9E;font-weight:600;padding:5px 10px;border-radius:6px;font-size:0.8rem;">
                                            -
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2 text-center">
                                    @if ($product->is_available)
                                        <span class="badge"
                                            style="background:#E6F9EE;color:#2D7A3B;font-weight:600;padding:5px 10px;border-radius:6px;font-size:0.8rem;white-space:nowrap;">
                                            Available
                                        </span>
                                    @else
                                        <span class="badge"
                                            style="background:#FFECEC;color:#C0392B;font-weight:600;padding:5px 10px;border-radius:6px;font-size:0.8rem;white-space:nowrap;">
                                            Unavailable
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2">
                                    <div class="btn-group" role="group" style="white-space:nowrap;">
                                        <a href="{{ route('owner.products.recipe', $product) }}" class="btn btn-sm"
                                            title="Recipe"
                                            style="background:#2D7A3B;color:#F5F0E5;border-radius:6px 0 0 6px;padding:5px 10px;font-weight:600;">
                                            <i class="bi bi-card-list"></i>
                                        </a>
                                        <a href="{{ route('owner.products.edit', $product) }}" class="btn btn-sm"
                                            title="Edit"
                ...
                <nav>
                    <ul class="pagination" style="gap:0.7rem; margin:0; padding:0; list-style:none; display:flex;">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link"
                                    href="{{ $products->appends(['search' => request('search'), 'category' => request('category')])->previousPageUrl() }}"
                                    rel="prev"
                        ...
                        @foreach ($products->links()->elements[0] as $page => $url)
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link"
                                        href="{{ $products->appends(['search' => request('search'), 'category' => request('category')])->url($page) }}"
                        ...
                        @if ($products->hasMorePages())
                            <li class="page-item">
                                <a class="page-link"
                                    href="{{ $products->appends(['search' => request('search'), 'category' => request('category')])->nextPageUrl() }}"
                                    rel="next"
